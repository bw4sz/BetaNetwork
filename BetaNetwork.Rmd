...
time_slices<-cut(indat,indat$Time,breaks=)

chains<-list()
traitfit<-list()
for (x in 2:length(time_slices)){
  
  #Run Model
  mod<-runModel(time_slices[[x]])
  
  #Get Chains
  chains[[x]]<-getChains(mod)
  
  #Retrieve Trait-match paramaters
  traitfit[[x]]<-getPar(mod)
