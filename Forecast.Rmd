...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    mtime<-system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/SpeciesIdentity_Detect_Elevation.jags",n.thin=1,n.iter=50000,n.burnin=49800,n.chains=2,DIC=F)
    )
    print(mtime)
    return(m2)
}
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    mtime<-system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/SpeciesIdentity_Detect_Elevation_Beta.jags",n.thin=1,n.iter=50000,n.burnin=49700,n.chains=2,DIC=F)
    )
    print(mtime)
    return(m2)
}
...
ggplot(seasonplot,aes(x=HLink)) + geom_pointrange(aes(y=mean,ymin=lower,ymax=upper,col=Season),position = position_dodge(width=0.2))  + labs(y="Probability of interaction",x="Link") + coord_flip() + theme_bw() + scale_color_manual(values=c("grey60","black"))
```
