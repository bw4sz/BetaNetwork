...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/SpeciesIdentity.jags",n.thin=1,n.iter=5000,n.burnin=4800,n.chains=2,DIC=F)
    )
    return(m2)
}
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    mtime<-system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/SpeciesIdentity_Detect_Elevation.jags",n.thin=1,n.iter=10000,n.burnin=9800,n.chains=2,DIC=F)
    )
    print(mtime)
    return(m2)
}
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    mtime<-system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/SpeciesIdentity_Detect_Elevation_Beta.jags",n.thin=1,n.iter=10000,n.burnin=9800,n.chains=2,DIC=F)
    )
    print(mtime)
    return(m2)
}
...
getChains<-function(mod){
  ...
  splitpc<-split(pc,pc$parameter)
    ...
    splitpc[c("alpha_occ","beta_occ","beta2_occ","detect")],function(x){
      ...
      splitpc[c("beta_season")]<-lapply(
  splitpc[c("beta_season")],function(x){
    sv<-data.frame(str_match(x$par,"(\\w+)\\[(\\d+),(\\d+)]"))[,3:4]
    colnames(sv)<-c("jBird","jPlant")
    pc<-data.frame(x,sv)
  })
...
```{r}
beta_season<-chains$Species_Detect_Elevation_Beta %>% filter(parameter %in% "beta_season") %>% mutate(Link=paste(jBird,jPlant,sep="_")) %>% group_by(jBird,Link) %>% dplyr::summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95))

#Which contain 0 effect
to_color<-beta_season %>% group_by(jBird,Link) %>% dplyr::summarize(sig=!(upper > 0 & lower < 0))
  
beta_season %>% inner_join(to_color) %>% arrange(desc(mean),desc(upper)) %>% ggplot(.,aes(x=Link)) + geom_linerange(aes(ymin=lower,ymax=upper,col=sig)) + coord_flip() + theme(axis.text.y=element_blank()) + labs(col="Effect of Season")
```
