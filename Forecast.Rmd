...
models$Random$Predicted<-runModel(Yobs_dat=Yobs_dat,Ynew_dat=Ynew_dat)
```
...
```{r}
chains$Random$Predicted<-getChains(models$Random$Predicted)
```
...
###Chains
chains$Random$Predicted %>% filter(parameter == "fitnew")  %>% ggplot(.,aes(x=Draw,y=value,col=as.factor(chain))) + geom_line()  + theme_bw() + labs(col="Chain") + facet_wrap(~parameter)
```

##Generate network

```{r}
posterior<-chains$Random$Predicted %>% filter(parameter %in% c("alpha")) %>% select(-par) %>% mutate(jBird=as.numeric(jBird),jPlant=as.numeric(jPlant)) %>% spread(parameter,value) %>% inner_join(jagsIndexBird) %>% inner_join(jagsIndexPlants)
...
genNetwork<-function(posterior){
  ...
  sppair<-Ynew_dat %>% distinct(Hummingbird,Iplant_Double)
  for(i in 1:nrow(sppair)){
    bird<-sppair$Hummingbird[i]
    plant<-sppair$Iplant_Double[i]
    samp<-posterior %>% filter(Hummingbird %in% bird ,Iplant_Double %in% plant) %>% sample_n(1)
    j<-genLink(alpha=samp$alpha)
    m[[i]]<-data.frame(Hummingbird=bird,Iplant_Double=plant,value=j)
  }
  m<-bind_rows(m)
  m<-acast(data=m,Hummingbird~Iplant_Double)

  return(m)
}

networks$Random$Predicted<-lapply(1:2,function(k) genNetwork(posterior = posterior))
```
...
```{r}
#Run Model
models$Random$Estimated<-runModel(Yobs_dat=Ynew_dat,Ynew_dat=Ynew_dat)

#Get Chains
chains$Random$Estimated<-data.frame(getChains(models$Random$Estimated))

chains$Random$Estimated %>% filter(parameter == "fitnew")  %>% ggplot(.,aes(x=Draw,y=value,col=as.factor(chain))) + geom_line()  + theme_bw() + labs(col="Chain") + facet_wrap(~parameter)
```

### Network

```{r}
posterior<-chains$Random$Estimated %>% filter(parameter %in% c("alpha")) %>% select(-par) %>% mutate(jBird=as.numeric(jBird),jPlant=as.numeric(jPlant)) %>% spread(parameter,value) %>% inner_join(jagsIndexBird) %>% inner_join(jagsIndexPlants)

networks$Random$Estimated<-lapply(1:2,function(k) genNetwork(posterior=posterior))
```
...
```{r,cache=FALSE,fig.width=13,fig.height=20}
###Chains
chains$Species$Predicted %>% filter(parameter == "fitnew")  %>% ggplot(.,aes(x=Draw,y=value,col=as.factor(chain))) + geom_line()  + theme_bw() + labs(col="Chain") + facet_wrap(~parameter)
```
...
networks$Species$Predicted<-lapply(1:2,function(k) genNetwork(posterior = posterior))
```
...
chains$Species$Estimated %>% filter(parameter == "fitnew")  %>% ggplot(.,aes(x=Draw,y=value,col=as.factor(chain))) + geom_line()  + theme_bw() + labs(col="Chain") + facet_wrap(~parameter)
```
...
networks$Species$Estimated<-lapply(1:2,function(k) genNetwork(posterior=posterior))
```
...
#Generate a network from the model
genNetwork<-function(posterior){
  ...
  for(i in 1:nrow(sppair)){
    ...
    j<-genLink(alpha=samp$alpha,beta=samp$beta1,traitdiff)
    m[[i]]<-data.frame(Hummingbird=bird,Iplant_Double=plant,value=j)
  }
  m<-bind_rows(m)
  m<-acast(data=m,Hummingbird~Iplant_Double)
...
networks$Traits$Predicted<-lapply(1:2,function(j) genNetwork(posterior=posterior))
```
...
networks$Traits$Estimated<-lapply(1:2,function(k) genNetwork(posterior=posterior))
```
...
allnetworks<-lapply(networks,function(x){
  lapply(x,function(y){
    lapply(y,function(z){
      d<-networklevel(z,index=c("connectance","niche overlap"),level="lower")
...
allnetworks<-allnetworks %>% select(Model=L1,Data=L2,Metric,value)
ggplot(allnetworks,aes(x=value)) + geom_density(aes(fill=Model),alpha=0.5) + facet_grid(Data~Metric)
```
...
```{r}
prob_matrix<-function(x,y){
  a_mat<-sum(x*y)
  c_mat<-sum(x*(1-y))
  b_mat<-sum((1-x)*y)
  list(a=a_mat,b=b_mat,c=c_mat)
}
```
...
```{r}
#Whittacker B0 in betalink
alphas<-list()

alphas$Random$Predicted<-chains$Random$Predicted %>% filter(parameter=="alpha") %>% group_by(jBird,jPlant) %>% summarize(value=mean(inv.logit(value))) %>% acast(jBird~jPlant,value.var="value")

alphas$Random$Estimated<-chains$Random$Estimated %>% filter(parameter=="alpha") %>% group_by(jBird,jPlant) %>% summarize(value=mean(inv.logit(value))) %>% acast(jBird~jPlant,value.var="value")

alphas$Species$Predicted<-chains$Species$Predicted %>% filter(parameter=="alpha") %>% group_by(jBird,jPlant) %>% summarize(value=mean(inv.logit(value))) %>% acast(jBird~jPlant,value.var="value")

alphas$Species$Estimated<-chains$Species$Estimated %>% filter(parameter=="alpha") %>% group_by(jBird,jPlant) %>% summarize(value=mean(inv.logit(value))) %>% acast(jBird~jPlant,value.var="value")

betameasures<-lapply(alphas,function(x){
  lapply(x,function(y){
    BW=B01(prob_matrix(y[[1]],y[[2]]))
    data.frame(BW=BW)
  })
})
