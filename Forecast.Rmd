...
runModel<-function(Yobs_dat,Ynew_dat,jTraitmatch){
  ...
  #MCMC options
    system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/TraitMatch.jags",n.thin=1,n.iter=10000,n.burnin=9000,n.chains=2,DIC=F)
    )
    return(m2)
}
...
Enew<-Ynew_dat %>% dplyr::select(-jinterval,-interval) %>% mutate(Index=1:nrow(Ynew_dat)) %>% inner_join(Enew,by="Index") %>% group_by(Model,Hummingbird,Iplant_Double) %>% summarize(mean=mean(inv.logit(value),na.rm=T)) 
```
