...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/SpeciesIdentity.jags",n.thin=1,n.iter=2000,n.burnin=1800,n.chains=2,DIC=F)
    )
    return(m2)
}
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/SpeciesIdentity_Detect.jags",n.thin=1,n.iter=3000,n.burnin=2800,n.chains=2,DIC=F)
    )
    return(m2)
}
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    mtime<-system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/SpeciesIdentity_Detect_Elevation.jags",n.thin=1,n.iter=3000,n.burnin=2800,n.chains=2,DIC=F)
    )
    print(mtime)
    return(m2)
}
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #Parameters to track
  ParsStage <- c("Ynew_pred","fitnew","Enew","alpha_occ","beta_occ","beta2_occ","detect","alpha")
  ...
  Dat<-list(
    Yobs=Yobs,
    Seasons=length(unique(indat$Season)),
    Birds=max(c(Yobs_dat$jBird,Ynew_dat$jBird)),
    Season=indat %>% ungroup() %>% select(jID,Season) %>% arrange(jID) %>% distinct(jID,Season) %>% .$Season,
    elevation=indat %>% ungroup() %>% select(jID,ele) %>% arrange(jID) %>% distinct(jID,ele) %>% .$ele,
    Bird=Yobs_dat$jBird,
    Plant=Yobs_dat$jPlant,
    Plants=max(c(Yobs_dat$jPlant,Ynew_dat$jPlant)),
  ...
  #MCMC options
    mtime<-system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/SpeciesIdentity_Detect_Elevation_Beta.jags",n.thin=1,n.iter=10,n.burnin=0,n.chains=2,DIC=F)
    )
    print(mtime)
    return(m2)
}
...
getChains<-function(mod){
  ...
  splitpc[c("alpha")],function(x){
    ...
    colnames(sv)<-c("jBird","jPlant","Season")
    pc<-data.frame(x,jBird=sv)
  })
