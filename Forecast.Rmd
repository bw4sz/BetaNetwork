...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/SpeciesIdentity.jags",n.thin=1,n.iter=30000,n.burnin=29500,n.chains=2,DIC=F)
    )
    return(m2)
}
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #Parameters to track
  ParsStage <- c("Ynew_pred","fitnew","Enew","alpha")
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  InitStage <- function(){
    ...
    list(dcam=initB,Ynew_pred=Ynew_pred,z=z)}
  
  #Parameters to track
  ParsStage <- c("Ynew_pred","fitnew","Enew","detect","alpha")
  ...
  #MCMC options
    system.time(
      m2<-jags(data=
      Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/SpeciesIdentity_Detect.jags",n.thin=2,n.iter=30000,n.burnin=29500,n.chains=2,DIC=F)
    )
    return(m2)
}
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  InitStage <- function(){
    ...
    list(dcam=initB,Ynew_pred=Ynew_pred,occ=occ,z=z)}
  
  #Parameters to track
  ParsStage <- c("Ynew_pred","fitnew","alpha","Enew","alpha_occ","beta_occ","detect")
  ...
  #MCMC options
    mtime<-system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/SpeciesIdentity_Detect_Elevation.jags",n.thin=1,n.iter=80000,n.burnin=79500,n.chains=2,DIC=F)
    )
    print(mtime)
    return(m2)
}
...
getChains<-function(mod){
  ...
  splitpc<-split(pc,pc$parameter)
    ...
    splitpc[c("alpha_occ","beta_occ","detect")]<-lapply(
    splitpc[c("alpha_occ","beta_occ","detect")],function(x){
      sv<-data.frame(str_match(x$par,"(\\w+)\\[(\\d+)]"))[,3]
      pc<-data.frame(x,jBird=sv)
    })
...
```{r}
elev_sim<-function(jBird,alpha,beta,elevation){
  p<-inv.logit(alpha+beta*elevation)
  data.frame(jBird=jBird,elevation=elevation,p=p)
}

bird_range <- chains$Species_Detect_Elevation %>% filter(parameter %in% c("alpha_occ","beta_occ")) %>% select(-par) %>% spread(parameter,value) %>% group_by(jBird,Draw,chain) %>% do(elev_sim(.$jBird,.$alpha_occ,.$beta_occ,elevation=unique(Ynew_dat$ele))) %>% mutate(jBird=as.numeric(as.character(jBird))) %>% inner_join(jagsIndexBird) %>% group_by(Hummingbird,elevation) %>% dplyr::summarise(mean=mean(p),lower=quantile(p,0.05),upper=quantile(p,0.95))
...
ggsave("Figures/ElevDetection.jpeg",height=7,width=11)
```
...
ggplot(conf_matrix %>% filter(variable == "accuracy"),aes(x=Model)) + geom_linerange(aes(ymin=lower,ymax=upper),size=2)  + coord_flip() + theme_bw() + labs(y="Prediction Accuracy") + scale_y_continuous(breaks = pretty_breaks())
...
## Rewiring: Change in the probability of species interaction 
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  InitStage <- function(){
    ...
    occ<-array(dim=c(Dat$Birds,Dat$Plants,Dat$Cameras),data=1)
    z<-array(dim=c(Dat$Birds,Dat$Plants,Dat$Cameras),data=1)
    list(dcam=initB,z=z,Ynew_pred=Ynew_pred,occ=occ)}
  
  #Parameters to track
  ParsStage <- c("Ynew_pred","Enew","alpha_occ","beta_occ","detect","alpha","beta_season")
  ...
  #MCMC options
    mtime<-system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/SpeciesIdentity_Detect_Elevation_Beta.jags",n.thin=1,n.iter=80000,n.burnin=79500,n.chains=2,DIC=F)
    )
    print(mtime)
    return(m2)
}
...
ggplot(seasonplot,aes(x=HLink)) + geom_pointrange(aes(y=mean,ymin=lower,ymax=upper,col=Season),position = position_dodge(width=0.5))  + labs(y="Probability of interaction",x="Link") + coord_flip() + theme_bw() 
ggsave("Figures/SpeciesBetadiversity.svg",height=3,width=6.5)
```
...
## Turnover: Change in the probability of species occurrence 
```{r}
#Source model
source("models/SpeciesIdentity_Detect_Elevation_SeasonOcc.R")

#print model
writeLines(readLines("models/SpeciesIdentity_Detect_Elevation_SeasonOcc.R"))
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #Parameters to track
  ParsStage <- c("Ynew_pred","Enew","alpha_occ","beta_occ","beta_int","detect","alpha","beta_season")
  ...
  #MCMC options
    mtime<-system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/SpeciesIdentity_Detect_Elevation_SeasonOcc.jags",n.thin=1,n.iter=80000,n.burnin=79500,n.chains=2,DIC=F)
    )
    print(mtime)
    return(m2)
}

models$Species_Detect_Elevation_SeasonOcc<-runModel(Yobs_dat=Yobs_dat,Ynew_dat=Ynew_dat)
```
...
getChains<-function(mod){
  ...
  splitpc<-split(pc,pc$parameter)
    ...
    splitpc[c("alpha_occ","beta_occ","beta_int","beta_season","detect")]<-lapply(
    splitpc[c("alpha_occ","beta_occ","beta_int","beta_season","detect")],function(x){
      sv<-data.frame(str_match(x$par,"(\\w+)\\[(\\d+)]"))[,3]
      pc<-data.frame(x,jBird=sv)
    })
...
```{r}
##Generate chains
chains$Species_Detect_Elevation_SeasonOcc<-getChains(models$Species_Detect_Elevation_SeasonOcc)

##Generate networks
networks$Species_Detect_Elevation_SeasonOcc<-getPredictions(chains$Species_Detect_Elevation_SeasonOcc,Ynew_dat = Ynew_dat)
```
...
```{r,cache=FALSE,fig.width=5,fig.height=2}
###Chains
chains$Species_Detect_Elevation_SeasonOcc %>% filter(parameter == "beta_int")  %>% ggplot(.,aes(x=Draw,y=value,col=as.factor(chain))) + geom_line()  + theme_bw() + labs(col="Chain") + facet_wrap(~jBird)
```

### Predicted elevation ranges

```{r}
elev_sim_int<-function(dat,elevation,seasons){
  out<-list()
  for (y in 1:length(seasons)){
      p=inv.logit(dat$alpha_occ + dat$beta_occ * elevation + dat$beta_season * seasons[y] + dat$beta_int * elevation * seasons[y])
        out[[y]]<-data.frame(jBird=unique(dat$jBird),alpha_occ=dat$alpha_occ,beta_occ=dat$beta_occ,beta_season=dat$beta_season,beta_int=dat$beta_int,season=seasons[y],elevation=elevation,p)
  }
  return(bind_rows(out))
}

bird_range_int <- chains$Species_Detect_Elevation_SeasonOcc %>% filter(parameter %in% c("alpha_occ","beta_occ","beta_int","beta_season")) %>% select(-par) %>% spread(parameter,value) %>% group_by(jBird,Draw,chain) %>% do(elev_sim_int(.,elevation=unique(Ynew_dat$ele),seasons=unique(Ynew_dat$jSeason))) %>% mutate(jBird=as.numeric(as.character(jBird))) %>% inner_join(jagsIndexBird) %>% group_by(Hummingbird,elevation,season) %>% dplyr::summarise(mean=mean(p),lower=quantile(p,0.05),upper=quantile(p,0.95))
 
#recode season
bird_range_int$season<-as.factor(bird_range_int$season)
levels(bird_range_int$season)<-c("Dry","Wet")

#Scientific names
ggplot(bird_range_int,aes(x=elevation,col=season))  + geom_line(aes(y=mean))+ geom_ribbon(aes(ymin=lower,ymax=upper,fill=season),alpha=0.2) + facet_wrap(~Hummingbird,ncol=4) + labs(x="Elevation",y="Probability of Occurrence") + theme_bw()
ggsave("Figures/Elevation_ranges_int.jpeg",height=8,width=11)
ggsave("Figures/Elevation_ranges_int.svg",height=8,width=11)
```

### Effect of season on occurrence probabilities

```{r}
beta_int<-chains$Species_Detect_Elevation_SeasonOcc %>% filter(parameter %in% "beta_int") %>% mutate(Link=paste(jBird,sep="_")) %>% group_by(jBird) %>% dplyr::summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95)) 

#Which contain 0 effect
to_color<-beta_int %>% group_by(jBird) %>% dplyr::summarize(sig=!(upper > 0 & lower < 0))

#Grab sig ones
beta_int<-beta_int %>% inner_join(to_color) %>% mutate(jBird=as.numeric(as.character(jBird))) %>% inner_join(jagsIndexBird)  %>% arrange(desc(mean)) 

#reorder
beta_int$Hummingbird<-factor(beta_int$Hummingbird,levels=beta_int$Hummingbird)

ggplot(beta_int,aes(x=Hummingbird)) + geom_linerange(aes(ymin=lower,ymax=upper,col=sig)) + coord_flip() + labs(col="Effect of Season")

#just plot the sig ones
toplot<-bird_range_int %>% dplyr::filter(Hummingbird %in% beta_int[beta_int$sig==T,"Hummingbird"]$Hummingbird)

ggplot(toplot %>% filter(!Hummingbird %in% c("Hoary Puffleg","Rufous-tailed Hummingbird")) ,aes(x=elevation,col=season))  + geom_line(aes(y=mean))+ geom_ribbon(aes(ymin=lower,ymax=upper,fill=season),alpha=0.2) + facet_wrap(~Hummingbird,ncol=3) + labs(x="Elevation",y="Probability of Occurrence",fill="Season",col="Season") + theme_bw() 
ggsave("Figures/Elevseason.svg",height=6,width=10)
ggsave("Figures/Elevseason.jpg",height=6,width=10)
```
...
betastat_occ<-chains$Species_Detect_Elevation_SeasonOcc %>% filter(parameter=="Ynew_pred") %>% mutate(Index=as.numeric(as.character(Index))) %>% group_by(Draw,chain) %>% do(pred_beta(.,Ynew_dat))
colnames(betastat_occ)<-c("Draw","chain","value","Betadiversity")

#recode levels to make more attractive.
betastat_occ$Betadiversity<-factor(betastat_occ$Betadiversity)
levels(betastat_occ$Betadiversity)[levels(betastat_occ$Betadiversity) %in% "WN"]<-"Interaction Dissimilarity"
levels(betastat_occ$Betadiversity)[levels(betastat_occ$Betadiversity) %in% "ST"]<-"Interaction Turnover"
levels(betastat_occ$Betadiversity)[levels(betastat_occ$Betadiversity) %in% "OS"]<-"Interaction Rewiring"
levels(betastat_occ$Betadiversity)[levels(betastat_occ$Betadiversity) %in% "S"]<-"Species Turnover"

ggplot(betastat_occ %>% filter(!Betadiversity %in% "Species Turnover"),aes(x=value)) + geom_density(alpha=0.4,fill="black") + facet_wrap(~Betadiversity,scales="free") + geom_vline(data=obs_dis  %>% filter(!Betadiversity %in% "Species Turnover"),aes(xintercept=value),linetype="dashed") + theme_bw()
ggsave("Figures/OccTurnover.svg",height=2,width=6)
ggsave("Figures/OccTurnover.jpeg",height=2,width=6)
```
...
samp<-allchains %>% filter(jBird %in% 2,parameter=="alpha") %>% group_by(Model,jPlant) %>% mutate(value=inv.logit(value))%>% dplyr::summarize(mean=mean(value),lower=quantile(value,0.05),upper=quantile(value,0.95))
ggplot(data=samp,aes(x=jPlant)) + geom_pointrange(aes(y=mean,ymin=lower,ymax=upper)) 
```
