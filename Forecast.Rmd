...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/SpeciesIdentity.jags",n.thin=1,n.iter=1000,n.burnin=500,n.chains=2,DIC=F)
    )
    return(m2)
}
...
runModel<-function(Yobs_dat,Ynew_dat,jTraitmatch){
  ...
  #MCMC options
    system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/TraitMatch.jags",n.thin=1,n.iter=1000,n.burnin=500,n.chains=2,DIC=F)
    )
    return(m2)
}
...
fitnew<-allchains %>% filter(parameter=="Enew") %>% mutate(Index=as.numeric(Index)) %>% select(Model,Draw,chain,Index,value) %>% inner_join(Ynew_dat,by="Index") %>% group_by(Model,Draw,chain,Yobs) %>% dplyr::summarize(p=sum(value)/n())

fitnew$Yobs<-factor(fitnew$Yobs)
levels(fitnew$Yobs)<-c("Absences","Presences")

ggplot(fitnew,aes(x=1-p)) + geom_density(aes(fill=Model),alpha=0.5) + theme_bw() + labs(x="Proportion of correctly predicted observations") + scale_x_continuous(labels=scales::percent) + facet_wrap(~Yobs,scales="free")  
...
ggplot(allnetworks,aes(x=value)) + geom_density(aes(fill=Model),alpha=0.5) + facet_wrap(~Metric) + geom_vline(data=obs_net,aes(xintercept=value),linetype="dashed") + theme_bw()
```
