...
runModel<-function(Yobs_dat,Ynew_dat,jTraitmatch){
  ...
  #MCMC options
    system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/TraitMatch_Detect.jags",n.thin=1,n.iter=5000,n.burnin=4900,n.chains=2,DIC=F)
    )
    return(m2)
}
...
runModel<-function(Yobs_dat,Ynew_dat){
  ...
  #MCMC options
    system.time(
      m2<-jags(data=Dat,parameters.to.save=ParsStage,inits=InitStage,model.file="models/SpeciesIdentity_Detect_Elevation.jags",n.thin=1,n.iter=4000,n.burnin=3900,n.chains=2,DIC=F)
    )
    return(m2)
}
